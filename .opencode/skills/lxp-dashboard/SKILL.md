...
1. Read the precomputed index at `scraped/raw/homework-index.json` (run `npm run index` first if it is
   missing or the scraped data changed).
2. Cross-reference `scraped/raw/content-tree.json` when detail on a specific item is needed.
...
- Local files live in `scraped/courses/*/files/`, named `{itemId}_*` (openable/linkable by itemId).
...
- Web app: LXP Homework — `npm run web` at the repo root (serves `apps/web` via `apps/server`).
