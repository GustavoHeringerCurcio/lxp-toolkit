...
2. ÍNDICE     (server)     npm run index:web     scraped/ -> apps/server/data/exercises.json
3. GERAÇÃO    (server)     botão "Gerar"         exercises + PDF -> OpenAI -> data/answers.json
...
- Leitura: só os passos 1 e 5 falam com o portal. O app lê do `scraped/` já raspado.
...
| Enviar upload | funciona (runner `packages/portal/scripts/submit-task.ts`, ação `upload`) |
...
| `src/send.ts` | dispara o runner de envio no pacote do portal (`packages/portal`) |
...
```bash
npm install
cp apps/server/.env.example apps/server/.env   # coloque OPENAI_API_KEY
npm run index:web                              # monta apps/server/data/exercises.json
npm run web                                    # build + servidor -> http://localhost:4174
npm run typecheck
```

Antes: `npm run dump` (raspa o portal via `packages/portal`).
