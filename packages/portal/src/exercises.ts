 ...
 * see packages/portal/docs/gaps.md. The functions below throw until then, so no accidental submission
 * can happen.
 */
...
function notCaptured(fn: string): never {
  throw new Error(
    `${fn}(): the quiz/upload SUBMIT endpoint has not been captured yet. ` +
      `See packages/portal/docs/gaps.md — run a controlled headful capture (npm run capture-api -- --url <item> --headful) ` +
